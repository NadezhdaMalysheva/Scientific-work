...
from process_counterparts import process_counterparts
...
def parse_cli_args():
    ...
    arg_type = str
    default_value = '/home/horungev/Catalogs/SRG/crossmatch/getaroundr.py'
...
def get_flags_data_path(check=False):
    ...
    if check:
        ...
        if errors:
            raise Exception(
                "Flags data is broken. Check hea134:/data/SRGz/pzph1/objids for correct flags data.")
...
def read_predictions_apart(path: str):
    ...
    for file in glob.glob(os.path.join(path, dataset_format.format('*'))):
        ...
        chunk_number = re.findall(dataset_format.format('(\d*)'), fname)
        if not chunk_number:
            continue
...
def assemble_and_analyze_results(buf_path: str, dst_path: str, models_series, file_name=None):
    ...
    for file in glob.glob(os.path.join(buf_path, '*.features.gz_pkl')) if file_name is None else [os.path.join(buf_path, file_name+'.features.gz_pkl')]:
        ...
        preds_dst_file = os.path.join(dst_path,
                                      f'{fname}.predictions.{models_series}.gz_pkl')
        best_dst_file = os.path.join(dst_path,
                                     f'{fname}.best.{models_series}.gz_pkl')

        preds = []
        for preds_file in glob.glob(
                os.path.join(buf_path, f'{fname}.p*.{models_series}*')):

            is_preds_file = re.findall(f'\.{models_series}\d\d\.', preds_file)
            # print(preds_file, is_preds_file, models_series)
            if is_preds_file:
