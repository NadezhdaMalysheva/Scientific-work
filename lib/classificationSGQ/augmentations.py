...
AUG_DATA_PATH = '/home/nmalysheva/task/S-G-Q_DESI+PanSTARRS+SDSS+WISE+J_UHS'
models = [
...
data = {
    'input': os.path.join(AUG_DATA_PATH, 'data/rename_x1_class.gz_pkl'),
    'one_gauss': os.path.join(AUG_DATA_PATH, 'data/one_gauss.gz_pkl'),
#     'input+one_gauss': './data/input+one_gauss.gz_pkl',
    'optimization': os.path.join(AUG_DATA_PATH, 'data/pred_optimization_full.gz_pkl'), #'data/pred_optimization_3.gz_pkl'),
    'test200': os.path.join(AUG_DATA_PATH, 'data/test200.gz_pkl') 
#     'input+optimization': './data/input+optimization.gz_pkl',
}

def read_data_version(version='input'):
    """
    Read data from version in ['input', 'one_gauss', 'optimization', 'test200']
    """
    return Catalog.read_table(data[version])
...
def preds(modelsSeries='x1'):
    return [f'zoo_best-{modelsSeries}_z_max', f'zoo_best-{modelsSeries}_z_maxConf']

ids = ['sdss_objID', 'ps_objID', 'ls_objid', 'ls_ebv'] + ['ps_w1flux', 'ps_w2flux','ps_dw1flux', 'ps_dw2flux', 'ls_mw_transmission_g', 'ls_mw_transmission_r', 'ls_mw_transmission_z', 'ls_mw_transmission_w1', 'ls_mw_transmission_w2', 'ls_mw_transmission_w3', 'ls_mw_transmission_w4']
...
def change_coord(df_before):
    ...
    df_bef = df_before.loc[:, set(keys + ids + fluxs + mag + mag_agr + gaia_features) & set(df_before.columns)]
...
def plt_version(version, x, y, ax=None, path_to_files=None, color=None, title=None, norm=None, xlim=[-0.5, 7], ylim=[10, 28.5], s=1, alpha=0.8, cmap='cool', colorbar=True, min_col=None, max_col=None):
    import inspect
    if ax is None:
    ...
    if isinstance(version, str):
        ...
        if os.path.isfile(path_to_files):
            df = Catalog.read_table(path_to_files)
        elif len(glob.glob(os.path.join(path_to_files, f'*{version}*.gz_pkl'))) > 1:
    ...
    if color is None:
        ...
        del df
        norm = colors.Normalize(vmin=min_col or np.min(color), vmax=max_col or np.max(color), clip=True) if norm is None else norm
    elif isinstance(color, list) or isinstance(color, type(np.array([]))) or isinstance(color, type(pd.Series([]))):
        norm = colors.Normalize(vmin=min_col or np.min(color), vmax=max_col or np.max(color)) if norm is None else norm
...
def print_area(df, cls_column='class', title='All', s=0.009, alpha=0.4):
    ...
    fig, axd = plt.subplot_mosaic([['Big','S'],
                                   ['Q', 'G']],
                              gridspec_kw=gs_kw, figsize=(13, 13),
                              constrained_layout=True)
    classs = {1:'S', 2:'Q', 3:'G'}
...
class Augmentation:
    ...
    def predict(self, data, dm=0, gauss_augm=True, use_def_statistic=False, count_iter=1, type_model='shift'):
        ...
        for i in tqdm.tqdm(range(count_iter), desc="Iteration") if count_iter > 1 else [1]:
            if self.debug:
                print(f'i = {i}')
            ...
            for flux_name in tqdm.tqdm(self.used_flux - set([flux_statistic]), 
                                       desc="flax") if count_iter <= 1 else self.used_flux - set([flux_statistic]):
                if self.debug:
                    print(flux_name)
    ...
    def _predict_for_flux(self, flux_name, df, gauss_augm):
        ...
        if self.debug:
            print('after', flux, np.sum(flux), err) 
...
def read_after_pzph_predict(path, version, features=[], modelsSeries='x1', cat_columns=True):
#     for_clf = ['sdssdr16_u_psf', 'sdssdr16_g_psf', 'sdssdr16_r_psf', 'sdssdr16_i_psf', 'sdssdr16_z_psf', 'sdssdr16_u_cmodel', 'sdssdr16_i_cmodel', 'sdssdr16_u-g_psf', 'sdssdr16_u-r_psf', 'sdssdr16_u-i_psf', 'sdssdr16_u-z_psf', 'sdssdr16_u_psf-cmodel', 'sdssdr16_g-i_psf', 'sdssdr16_g_psf-cmodel', 'sdssdr16_r-i_psf', 'sdssdr16_i-z_psf', 'sdssdr16_i_psf-cmodel', 'psdr2_i_kron', 'psdr2_y_kron', 'psdr2_g_psf', 'psdr2_r_psf', 'psdr2_i_psf', 'psdr2_z_psf', 'psdr2_y_psf', 'psdr2_g-i_psf', 'psdr2_g-y_psf', 'psdr2_r-i_psf', 'psdr2_r-y_psf', 'psdr2_i-z_psf', 'psdr2_i-y_psf', 'psdr2_i_psf-kron', 'psdr2_z-y_psf', 'psdr2_y_psf-kron', 'decals8tr_Lw1-Lw2', 'decals8tr_Lw1', 'decals8tr_Lw2', 'decals8tr_g', 'decals8tr_r', 'decals8tr_z', 'decals8tr_g-r', 'decals8tr_g-z', 'decals8tr_r-z', 'psdr2_g_kron-decals8tr_Lw1', 'psdr2_g_kron-decals8tr_Lw2', 'psdr2_r_kron-decals8tr_Lw1', 'psdr2_r_kron-decals8tr_Lw2', 'psdr2_i_kron-decals8tr_Lw1', 'psdr2_i_kron-decals8tr_Lw2', 'psdr2_z_kron-decals8tr_Lw1', 'psdr2_z_kron-decals8tr_Lw2', 'psdr2_y_kron-decals8tr_Lw1', 'psdr2_y_kron-decals8tr_Lw2', 'sdssdr16_u_cmodel-decals8tr_Lw1', 'sdssdr16_u_cmodel-decals8tr_Lw2', 'sdssdr16_g_cmodel-decals8tr_Lw1', 'sdssdr16_g_cmodel-decals8tr_Lw2', 'sdssdr16_r_cmodel-decals8tr_Lw1', 'sdssdr16_r_cmodel-decals8tr_Lw2', 'sdssdr16_i_cmodel-decals8tr_Lw1', 'sdssdr16_i_cmodel-decals8tr_Lw2', 'sdssdr16_z_cmodel-decals8tr_Lw1', 'sdssdr16_z_cmodel-decals8tr_Lw2', 'sdssdr16_g_cmodel-decals8tr_g', 'sdssdr16_r_cmodel-decals8tr_r', 'sdssdr16_z_cmodel-decals8tr_z']
    ...
    for file in glob.glob(os.path.join(path, f'df_augg_cls_*{version}*features.gz_pkl')):
        ...
        tmp = pd.concat(
            ...
            axis=1)
        if cat_columns:
            tmp = tmp[sorted(list(set(tmp.columns) & set(keys + ids + features + mag + mag_agr + fluxs + gaia_features + preds(modelsSeries) + (cat_columns if isinstance(cat_columns, list) else []))))]
        df = pd.concat([df, tmp], axis=0)
    return df

def pzph_predict_SGQ(version='one_gauss', path_buf='__buf__', 
                     modelsIds=[21, 35], modelsSeries='x1', njobs=24, 
                     data=None, do_split=True, do_predict=True, do_assemble_and_analyze_results=True,
                     fold='', cat_columns=True):
    os.makedirs(os.path.join(path_buf, f'preds_{version}'), exist_ok=True)
    if do_split:
        print('split')
        for cls in [1, 2, 3]:
            if data is None:
                ...
                df_tmp = data.loc[data['class'] == cls] if 'class' in data else data
    ...
    os.makedirs(os.path.join(data_path, f'end'), exist_ok=True)

    
    if do_assemble_and_analyze_results:
        ...
        for cls in [1, 2, 3]:
            for file in glob.glob(os.path.join(data_path, f'df_augg_cls_{cls}*{version}*features.gz_pkl')):
                assemble_and_analyze_results(data_path, 
                                             os.path.join(data_path, f'end'), 
                                             models_series=modelsSeries, 
                                             file_name=file.split('/')[-1].split('.')[0], fold=fold)
    
        return read_after_pzph_predict(os.path.join(data_path, f'end'), version, modelsSeries=f'{modelsSeries}{fold}', cat_columns=cat_columns)  
    return
